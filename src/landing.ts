...
function clearFormInputs(): void {
  ...
  const signupFirstNameInput = document.getElementById(
    "signupFirstName"
  ) as HTMLInputElement;
  const signupLastNameInput = document.getElementById(
    "signupLastName"
  ) as HTMLInputElement;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  (document.getElementById("features-link") as HTMLElement).addEventListener(
    "click",
    () => smoothScroll("features-section")
  );
  (document.getElementById("operations-link") as HTMLElement).addEventListener(
    "click",
    () => smoothScroll("operations-section")
  );
  (
    document.getElementById("testimonials-link") as HTMLElement
  ).addEventListener("click", () => smoothScroll("testimonials-section"));
  ...
  function handleMouseEnter(event: Event) {
    console.log("mouse enter fun")
    const hoveredId = (event.currentTarget as HTMLElement).id;
    console.log(hoveredId);
    featureElements.forEach((id) => {
      const element = document.getElementById(id);
      if (element && element.id !== hoveredId) {
